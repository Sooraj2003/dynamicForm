...
import 'react-json-pretty/themes/monikai.css'; // Import a theme for the JSON viewer
...
const JsonEditor: React.FC<JsonEditorProps> = ({ onChange, mode }) => {
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (isValidJson(newJson)) {
      setError('');
      onChange(newJson); // Trigger parent onChange only if valid
    } else {
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col lg:flex-row h-full">
        ...
        <div className="w-full lg:w-1/2 p-4 border-l lg:border-l-2">
          {isValidJson(json) ? (
            <ReactJson
            data={JSON.parse(json)} // Use `data` instead of `src`
            // collapsed={false}
